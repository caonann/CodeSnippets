...
class Child : public Base {
 public:
  ...
  void printSomeThing() /*final*/ { printf("this is Child\n"); }
};

class ChildChild : public Child {
 public:
  // override声明了该函数一定是基于基类的重载，避免了书写上的错误和继承关系层数太多导致书写错误
  void printSomeThing() override { printf("this is ChildChild\n"); }

  // 函数重载，函数名，形参列表
  void printSomeThing(int a, int b) { std::cout << a << b << std::endl; }
};
